...
def ganache_cmd(args: GanacheInput) -> str:
    ...
    env_utilities.open_and_create_parent_dirs(args.configoutputfile)
    cmd = " ".join([
        "ganache-cli",
        block_delay,
        "-h 0.0.0.0",
        ...
        f"--port {args.port}",
        f"--account_keys_path {args.configoutputfile}",
        f"--db {args.db_dir}",
...
def start_ganache(args: GanacheInput):
    ...
    with env_utilities.open_and_create_parent_dirs(args.logfile) as logfile:
        ...
        keys = env_utilities.read_json_file(args.configoutputfile)
